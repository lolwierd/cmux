...
Rust is a required macOS build dependency. `rust-toolchain.toml` pins Rust 1.88.0, including both Apple targets. Every setup, CI, generation, benchmark, and Xcode command runs Cargo through that exact rustup toolchain with `--locked`. Release builds use size optimization, fat LTO, one codegen unit, isolated per-architecture target directories, and `MACOSX_DEPLOYMENT_TARGET=14.0`. The build leaves Cargo's intermediate proc-macro dylibs unstripped, then uses Apple's system `strip` on the final sidecar so macOS 27 can load those proc-macros during the build while the shipped artifact remains symbol-free.
